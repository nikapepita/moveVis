...
add_text_3D <- function(frames, labels, x, y, colour = "black", size = 3, type = "text", verbose = TRUE){
  ...
  add_gg(frames, rgl=function(i){text3d(x = data[[i]]$V2,y=data[[i]]$V3,text=data[[i]]$V1, adj = c(0.5, 17),col="black")} , gg = expr(annotate(type, x = data[[2]], y = data[[3]], label = data[[1]], colour = data[[4]], size = data[[5]])), data = data, type = type)
